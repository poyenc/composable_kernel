    ...
    def get_pipelines(dtype, hdim) -> List[FmhaFwdSplitKVPipeline]:
        ...
        if dtype in ['fp16', 'bf16']:
            for logits, mask, bias, pagedkv in itertools.product(["t", "f"], get_mask_map(mask_impl).keys(), BIAS_MAP.keys(), ["t", "f"]):
                pipelines.append(Pipeline('qr_nwarp_sshuffle', 'row', 'f', 't', 'f', 'f', logits, bias, 't', squant, pagedkv, mask))
                # pipelines.append(Pipeline('qr_nwarp_sshuffle', 'col', 'f', 't', 'f', 'f', logits, bias, 't', squant, pagedkv, mask))

                pipelines.append(Pipeline('qr_nwarp_sshuffle', 'row', 't', 'f', 'f', 'f', logits, bias, 't', squant, pagedkv, mask))
                # pipelines.append(Pipeline('qr_nwarp_sshuffle', 'col', 't', 'f', 'f', 'f', logits, bias, 't', squant, pagedkv, mask))

                pipelines.append(Pipeline('qr_nwarp_sshuffle', 'row', 't', 't', 'f', 'f', logits, bias, 't', squant, pagedkv, mask))
                # pipelines.append(Pipeline('qr_nwarp_sshuffle', 'col', 't', 't', 'f', 'f', logits, bias, 't', squant, pagedkv, mask))

                # pipelines.append(Pipeline('qr_nwarp_sshuffle', 'row', 't', 't', 't', 't', logits, bias, 't', squant, pagedkv, mask))
                # pipelines.append(Pipeline('qr_nwarp_sshuffle', 'col', 't', 't', 't', 't', logits, bias, 't', squant, pagedkv, mask))
