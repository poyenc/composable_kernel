...
float fmha_fwd(fmha_fwd_traits traits, fmha_fwd_args args, const ck_tile::stream_config& config) {{
    ...
    const bool can_dispatch_v3 =
        ...
        (traits.bias_type == bias_enum::no_bias) and
    ...
    def get_pipelines(
        ...
        if dtype in cls._DT_FP16_BF16:
            ...
            if (hdim, hdim_v) == (128, 128):
                ...
                for mask, lse in itertools.product(["no", "causal"], ["t", "f"]):
                    pipelines.append(FmhaFwdPipeline("qr_async_trload_v3", "row", "t", "t", "f", "f",
                        F_logits="f", F_bias="no", F_lse=lse, F_dropout="f", F_squant=squant, F_mask=mask, F_skip="f", F_trload="t"))  # fmt: skip
        return pipelines
