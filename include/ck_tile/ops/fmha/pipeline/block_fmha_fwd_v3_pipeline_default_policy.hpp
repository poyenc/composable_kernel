...
struct BlockFmhaV3PipelineDefaultPolicy
{
    ...
    CK_TILE_DEVICE static constexpr auto MakeKLdsStoreBlockDescriptor()
    {
        ...
        constexpr auto k_lds_block_desc_0 = make_naive_tensor_descriptor(
            make_tuple(number<NumWarps>{},
                       number<NThreadPerWarp>{},
                       number<KThreadPerWarp>{},
                       number<kKPack>{}),
            make_tuple(number<NThreadPerWarp * KThreadPerWarp * kKPack + NumElemsInPad>{},
        ...
        constexpr auto k_lds_block_desc = transform_tensor_descriptor(
            k_lds_block_desc_0,
            make_tuple(
                make_merge_transform(make_tuple(number<NumWarps>{}, number<NThreadPerWarp>{})),
    ...
    CK_TILE_DEVICE static constexpr auto MakeKLdsLoadBlockDescriptor()
    {
        ...
        return transform_tensor_descriptor(
            ...
            make_tuple(make_functor_transform(
                           [](auto idx) {
#if 1
                               return bit_cast<index_t>(swap_bit1_bit2(bit_cast<uint32_t>(idx)));
#else
#if 1
    ...
    CK_TILE_DEVICE static constexpr auto MakeVLdsStoreBlockDescriptor()
    {
...
#if 0
        return make_naive_tensor_descriptor(make_tuple(number<kKPerBlock>{}, number<kNPerBlock>{}),
                                            make_tuple(number<kNPerBlock>{}, number<1>{}),
                                            number<kKPack>{},
                                            number<1>{});
#else
        constexpr index_t NumWarps       = Problem::BlockFmhaShape::NumWarps;
        constexpr index_t NThreadPerWarp = kNPerBlock / kKPack;
        ...
        static_assert(NumWarps == 8);
        static_assert(KThreadPerWarp == 4);
        static_assert(NThreadPerWarp * KThreadPerWarp == 64);
        ...
        constexpr auto v_lds_block_desc_0 = make_naive_tensor_descriptor(
            make_tuple(number<NumWarps>{},
                       ...
                       number<NThreadPerWarp>{},
                       number<kKPack>{}),
            make_tuple(number<KThreadPerWarp * NThreadPerWarp * kKPack + NumElemsInPad>{},
                       number<NThreadPerWarp * kKPack>{}, // NThreadPerWarp
                       number<kKPack>{},                  // KThreadPerWarp
                       number<1>{}),                      // kKPack
            number<kKPack>{},
            number<1>{});
        ...
        auto v_lds_block_desc = transform_tensor_descriptor(
            v_lds_block_desc_0,
            make_tuple(
                make_merge_transform(make_tuple(number<NumWarps>{}, number<KThreadPerWarp>{})),
    ...
    {
        static_assert(MakeKLdsStoreBlockDescriptor<Problem>().get_element_space_size() ==
                      MakeKLdsLoadBlockDescriptor<Problem>().get_element_space_size());

        constexpr index_t k_element_space_size =
            MakeKLdsLoadBlockDescriptor<Problem>().get_element_space_size();
        static_assert(
            k_element_space_size * sizeof(typename Problem::KDataType) + kKLdsPadInBytes == 8320);
        return k_element_space_size * sizeof(typename Problem::KDataType) + kKLdsPadInBytes;
    }

    template <typename Problem>
    CK_TILE_DEVICE static constexpr ck_tile::index_t GetSmemSizeV()
    {
        static_assert(MakeVLdsStoreBlockDescriptor<Problem>().get_element_space_size() ==
                      MakeVLdsLoadBlockDescriptor<Problem>().get_element_space_size());

        constexpr index_t v_element_space_size =
            MakeVLdsLoadBlockDescriptor<Problem>().get_element_space_size();
        static_assert(
            v_element_space_size * sizeof(typename Problem::VDataType) + kVLdsPadInBytes == 8704);
        return v_element_space_size * sizeof(typename Problem::VDataType) + kVLdsPadInBytes;
    }
