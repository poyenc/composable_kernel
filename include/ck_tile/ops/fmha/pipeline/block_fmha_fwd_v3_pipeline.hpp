...
struct BlockFmhaFwdV3Pipeline
{
               ...
               void* __restrict__ smem_ptr) const
    {
        ...
        auto k_lds_window_load = generate_tuple(
            [&](auto read_idx) {
                auto k_buf = (read_idx == 0 ? smem_k0 : smem_k1);
                make_tile_window(
                    make_lds_tile_window(k_buf,
        ...
        auto v_lds_window_load = generate_tuple(
            [&](auto read_idx) {
                auto v_buf = (read_idx == 0 ? smem_v0 : smem_v1);
                make_tile_window(
                    make_lds_tile_window(v_buf,
        ...
        static_for<0, 2, 1>{}([&](auto idx) {
            k_lds_window_store(idx) = make_lds_tile_window(
                [&] {
                    if constexpr(idx == 0)
                        return smem_k0;
                    else
        ...
        static_for<0, 2, 1>{}([&](auto idx) {
            v_lds_window_store(idx) = make_lds_tile_window(
                [&] {
                    if constexpr(idx == 0)
                        return smem_v0;
                    else
