...
struct CoreLoopSchedulerDefaultBase
{
    ...
    {
        // GEMM0: 16 MFMA, ~34 VALU, ~16 TRANS (deferred exp2 second half)
        static_for<0, Params::kMfmaPerWarpGemm0, 1>{}([&](auto) {
            __builtin_amdgcn_sched_group_barrier(LLVMSchedGroupMask::MFMA, 1, 0);
            __builtin_amdgcn_sched_group_barrier(LLVMSchedGroupMask::TRANS, 1, 0);
    ...
    {
        // GEMM1: 16 MFMA, ~109 VALU, ~33 TRANS (exp2)
        // Explicitly schedule TRANS to distribute exp2 across MFMAs
        // (without this, compiler bunches all exp2 at end of cluster)
...
struct BlockFmhaFwdV3Pipeline
{
               ...
               void* __restrict__ smem_ptr) const
    {
        ...
        auto fmha_alu0_max = [&](auto sp_reg_idx) {
            ...
            int32x2_t swapped_regs =
                __builtin_amdgcn_permlane32_swap(bit_cast<int32_t>(m_latest.thread_buf_[0]),
                                                 ...
                                                 false,
                                                 false);
            // Use inline asm v_max_f32 to avoid 2 redundant NaN-safe v_max x,x
            // that __builtin_fmaxf generates. Safe because all 64 lanes are always
            // active, so v_permlane32_swap never produces NaN.
            {
                float a = bit_cast<float>(swapped_regs.x);
                float b = bit_cast<float>(swapped_regs.y);
                float result;
                asm volatile("v_max_f32 %0, %1, %2" : "=v"(result) : "v"(a), "v"(b));
                m_latest.thread_buf_[0] = result;
            }
#else
        ...
        // exp2(sp_compute) first half — stays in GEMM1 clusters (2/6)
        auto fmha_alu1_exp2_first = [&](auto sp_reg_idx) {
            constexpr index_t buf_size =
                std::decay_t<decltype(sp(sp_reg_idx).sp_compute)>::get_thread_buffer_size();
            constexpr index_t half = buf_size / 2;
            static_for<0, half, 1>{}([&](auto idx) {
                sp(sp_reg_idx).sp_compute.thread_buf_[idx] =
                    ck_tile::exp2(sp(sp_reg_idx).sp_compute.thread_buf_[idx]);
            });
        };

        // exp2(sp_compute) second half — deferred to GEMM0 clusters (0/4)
        auto fmha_alu1_exp2_second = [&](auto sp_reg_idx) {
            constexpr index_t buf_size =
                std::decay_t<decltype(sp(sp_reg_idx).sp_compute)>::get_thread_buffer_size();
            constexpr index_t half = buf_size / 2;
            static_for<half, buf_size, 1>{}([&](auto idx) {
                sp(sp_reg_idx).sp_compute.thread_buf_[idx] =
                    ck_tile::exp2(sp(sp_reg_idx).sp_compute.thread_buf_[idx]);
            });
        };
        ...
        auto core_loop = [&]() {
            ...
            // --- Cluster 0: GEMM0(sp[1]) + fmha_alu1_rest(sp[0]) + logits_trans(sp[1]) ---
            {
                ...
                fmha_alu1_exp2_second(number<0>{});
            ...
            // --- Cluster 2: step_k(0) + alu0_max + rescale | step_k(1-3) + sub + exp2 ---
            {
                ...
                // subtraction + exp2 first half (in step_k(1-3) MFMA shadow)
                fmha_alu0_sub(number<1>{});
                fmha_alu1_exp2_first(number<1>{});

                // scheduling for step_k(1-3): 6 pairs of {1 MFMA, 5 VALU}
                // + 3 pairs of {1 MFMA, 3 TRANS} for exp2 first half
                static_for<0, 6, 1>{}([&](auto) {
                    __builtin_amdgcn_sched_group_barrier(LLVMSchedGroupMask::MFMA, 1, 0);
                    __builtin_amdgcn_sched_group_barrier(LLVMSchedGroupMask::VALU, 5, 0);
                });
                static_for<0, 3, 1>{}([&](auto) {
                    __builtin_amdgcn_sched_group_barrier(LLVMSchedGroupMask::MFMA, 1, 0);
            ...
            // --- Cluster 4: GEMM0(sp[0]) + fmha_alu1_rest(sp[1]) + logits_trans(sp[0]) ---
            {
                ...
                fmha_alu1_exp2_second(number<1>{});
