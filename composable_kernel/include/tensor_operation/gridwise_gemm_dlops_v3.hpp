...
struct GridwiseGemmDlops_km_kn_mn_v3
{
...

    {
        ...
        static_for<0, HoPerThread, 1>{}([&](auto hi) {
            static_for<0, WoPerThread, 1>{}([&](auto wi) {
                auto max_x = c_thread_buf[Number<c_k1_n_h2_w2_thread_gemm_desc.CalculateOffset(
                    make_tuple(0, 0, hi, wi))>{}];
                static_for<1, KPerThread, 1>{}([&](auto ki) {
    ...
    __device__ static void ActivOp(CThreadBuff& c_thread_buf,
